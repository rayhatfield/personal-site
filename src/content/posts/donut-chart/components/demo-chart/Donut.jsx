...
export default class PieChart extends React.Component {
  ...
  makeSegment = ({ paths, subtotal }, { percent, color }, i) => {
    ...
    paths.push(
      <path
        key={i}
        className={styles.donutChartSegment}
        {...fillProp}
        stroke="none"
        d={commands.join(" ")}
      />
  ...
  render() {
    ...
    return !(items || []).length ? null : (
      <div className={styles.donutChartContainer}>
        {title && <div className={styles.donutChartTitle}>{title}</div>}
        <div className={styles.donutChart}>
          <svg viewBox={`0 0 ${size} ${size}`}>
            {items.reduce(this.makeSegment, { paths: [], subtotal: 0 }).paths}
          </svg>
        </div>
        <ul className={styles.donutChartLegend}>
          {items.map(({ value, percent, label }) => (
            <li key={`${value}-${label}`}>
              <span className={styles.donutChartLegendLabel}>{label}</span>
              {/* <span className={cx('donut-chart-legend-value', legendValue)}>{value}</span> */}
              <span className={styles.donutChartLegendPercent}>
